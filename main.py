...
def main():
    ...
    st.header("Q/A with your file with Qdrant and OpenAI")
    flag = False
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ Features", expanded=True):
            ...
            if flag:
                score_threshold = st.slider(label="Confidence Level", min_value=0.0, max_value=1.0, value=0.65, step=0.01)
                temperature = st.slider(label="Creativity Level", min_value=0.0, max_value=1.5, value=0.7, step=0.1)
            else:
                score_threshold = st.slider(label="Confidence Level(%)", min_value=0, max_value=100, value=65, step=1)
                score_threshold = round(score_threshold*0.0025, 1)
                temperature = st.slider(label="Creativity Level(%)", min_value=0, max_value=100, value=60, step=1)
                temperature = round(temperature*0.012, 1)
    ...
    if process:
        ...
        st.session_state.conversation = get_qa_chain(vectorestore, chat_model_type, temperature, score_threshold) #for openAI
...
def get_qa_chain(vectorstore, chat_model_type, temperature, score_threshold):
    # prompt_template = """
    ...
    qa = RetrievalQA.from_chain_type(llm=ChatOpenAI(model = chat_model_type, temperature=temperature), chain_type="stuff",
                                retriever=vectorstore.as_retriever(search_type="similarity_score_threshold",
                                                            search_kwargs={'score_threshold': score_threshold}),  return_source_documents=True)
    return qa
